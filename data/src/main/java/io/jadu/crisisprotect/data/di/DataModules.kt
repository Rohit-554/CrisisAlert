package io.jadu.crisisprotect.data.di
...
val dataModules: List<Module> = listOf(
    module {
        ...
        single { OkHttpClient.Builder().connectTimeout(10, TimeUnit.SECONDS).readTimeout(15, TimeUnit.SECONDS).callTimeout(20, TimeUnit.SECONDS).build() }
        single(named("usgsRetrofit")) { Retrofit.Builder().baseUrl(UsgsBaseUrl).client(get()).addConverterFactory(get<Json>().asConverterFactory("application/json".toMediaType())).build() }
    ...
    module {
        single { Room.databaseBuilder(androidContext(), CrisisProtectDatabase::class.java, "crisis_protect.db").addMigrations(migration1To2).build() }
